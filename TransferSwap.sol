...
import "sgn-v2-contracts/contracts/interfaces/IWETH.sol";
import "sgn-v2-contracts/contracts/interfaces/IUniswapV2.sol";
...
contract TransferSwap is MessageSenderApp, MessageReceiverApp{
    ...
    function transferWithSwap(
        ...
        _transferWithSwap(
            ...
            _srcSwap,
            _dstSwap,
            _maxBridgeSlippage,      //(100% - x)* amount
            _nonce,
            false,           // 
            msg.value       //付给MessageBus 费用
        );
    }
    ...
    function _transferWithSwap(
        ...
        // swap source token for intermediate token on the source DEX   将源代币换成中间代币
        ...
        if (_dstChainId == chainId) {   //不需要桥  直接transfer
    ...
    function executeMessageWithTransfer(
        ...
        if (m.swap.path.length > 1) {//2
            ...
            if (ok) {
                ...
                // handle swap failure, send the received token direct ly to receiver
